...
| `05_access.sql` | 43 `ACCESS` views: 20 plain `<RESOURCE>_VIEW`s (1:1 with FOUNDATION), ~17 `PATIENT_<RESOURCE>_VIEW`s joining each clinical resource back to its patient, 4 `ENCOUNTER_<RESOURCE>_VIEW`s ("what happened during this visit"), `CLAIM_EXPLANATION_OF_BENEFIT_VIEW` (billing chain), and `PATIENT_360_VIEW` (per-patient counts across every resource type). |
...
| `40_reconciliation_verify.sql` | Read-only verification queries (row counts, samples, mismatches-only, coverage) across every layer — also the query surface the Poc.html Reconciliation Agent is meant to use. |
